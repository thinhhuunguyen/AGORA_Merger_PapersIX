...
for idx in range(idx_begin - step, idx_eval + step, step):
    ...
    matching_pos = newstars_pos[np.intersect1d(track_sID, newstars_ID, return_indices=True)[2]]
    matching_ID = newstars_ID[np.intersect1d(track_sID, newstars_ID, return_indices=True)[2]]
    matching_pos = (matching_pos*ds.units.code_length).to('kpc').v
    matching_pos = matching_pos - center
    for k in range(len(matching_ID)):
        output[matching_ID[k]]['firststar_pos'] = matching_pos[k]
    #
