...
def calc_gas_mass_frac(idx, codetp, pfs, hullv):
    ...
    if codetp == 'ART' or codetp == 'ENZO' or codetp == 'RAMSES':
        ...
        sf_bool = prog_gasnH > 1
        cool_bool = prog_gastemp < 10**(4.5)
        
        gas_mass_total = prog_gasmass[prog_bool].sum()
        gas_mass_sf = prog_gasmass[prog_bool*sf_bool].sum()
        gas_mass_cool = prog_gasmass[prog_bool*cool_bool].sum()
        del prog_gascoor, prog_gasmass, prog_gastemp, prog_gasnH, prog_bool, sf_bool
        
        all_mass = prog_reg['all', 'particle_mass'].to('Msun').v
        all_pos = prog_reg['all', 'particle_position'].to('m').v
        all_bool = in_hull(all_pos, hullv[idx][prog_branch])
        del all_pos
    ...
    #-------------------------------------------------------------------------------------
        ...
        gas_mass_total = gasmass[prog_bool].sum()
        gas_mass_sf = gasmass[prog_bool*sf_bool].sum()
        gas_mass_cool = gasmass[prog_bool*cool_bool].sum()
...
if yt.is_root():
    ...
    np.save('/work/hdd/bezm/tnguyen2/AGORA/analysis/GasMassFraction_ProgBranch-%s_%s_ver%s_ConvexHull_preInfall_ver3.npy' % (merger_number, codetp, halotree_ver), output)
